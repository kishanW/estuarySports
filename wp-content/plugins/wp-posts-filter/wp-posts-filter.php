...
class wp_posts_filter {
	...
	public function wppf_settings_page( ) {
		...
		foreach ( $custom_options as $custom_opt => $title ) {
			...
			// echo '<span class="to-be-obsolete-soon">' . (isset( self::$wppf_opts[$custom_opt] ) ? self::$wppf_opts[$custom_opt] : 'None set') . '</span>';
		}
		echo '</div>' . PHP_EOL;
		...
		echo '<div id="save-button-placeholder" align="center">' . PHP_EOL;
		...
		if ( isset( $wppf_opts[$post->ID]['filterby'] ) ) {
			...
			if ( count( $filtered_posts->posts ) > 0 ) {
				...
				foreach ( $filtered_posts->posts as $entry ) {
					...
					$item = str_replace( self::$ph_date, get_the_date( $date_format ), $item );
